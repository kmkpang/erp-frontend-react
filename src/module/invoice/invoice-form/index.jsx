...
const InvoiceFormModal = ({
	...
	const [formData, setFormData] = useState(() => {
		if (initialData && isEditMode) {
			...
			const total =
				...
				(initialData.details || []).reduce((sum, p) => sum + (parseFloat(p.sale_price) || 0), 0);
			let vat = 0;
	...
	const isDerivedFromQuotation =
		!!formData.sale_id &&
		(!formData.quotation_num || !String(formData.quotation_num).includes("QT-AUTO"));
												...
												<div className="card-body py-3">
													...
													{loadingDeposit ? (
														...
														<div className="d-flex gap-3 mb-3">
															{depositSummary?.total_deposit_invoiced > 0 ? (
																<div className="form-check">
																	<input
																		className="form-check-input"
																		type="radio"
																		name="deposit_type"
																		id="deposit_type_remaining"
																		value="full"
																	...
																	<label
																		...
																		htmlFor="deposit_type_remaining"
																	>
																		ยอดคงเหลือ
																	</label>
																</div>
