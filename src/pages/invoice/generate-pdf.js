...
export const generatePDF = async (
	...
	// Document Number
	const docNumber = row.invoice_number || "-";
	...
	const renderHeader = () => {
		...
		doc.setFont("THSarabunNew", "normal");
		doc.setFontSize(16);
		doc.text("ใบแจ้งหนี้", 170, 20, { align: "center" });
	...
	const renderSectionBoxes = () => {
		...
		doc.setFont("THSarabunNew", "normal");
		doc.text(docNumber, 170, 73);
		doc.text(displayDate, 170, 87);
		doc.text(`${referenceLabel} ${referenceText}`, 145, 100);
	};

	// Calculations for footer
	const price = parseFloat(row.total_grand || row.total_price || row.sale_totalprice || 0);
	...
	const renderFooter = (lastY) => {
		...
		// Check if we have enough space for the summary/signatures (~90 units)
